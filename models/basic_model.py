...
class IDCM_NN(nn.Module):
    ...
    def __init__(self, img_input_dim=4096, img_output_dim=2048,
        ...
        self.linearLayer1 = nn.Linear(img_output_dim, minus_one_dim) 
        # self.linearLayer2 = nn.Linear(minus_one_dim, 64)
        self.co_attention = Attention_Layer(minus_one_dim)
        self.classifier = nn.Linear(minus_one_dim, output_dim)

    def forward(self, img, text):
        view1_feature = self.img_net(img)
        view2_feature = self.text_net(text)
        view1_feature = self.linearLayer1(view1_feature)
        # view1_feature = self.linearLayer2(view1_feature)
        view2_feature = self.linearLayer1(view2_feature)
        # fusion_feat = torch.add(view1_feature, view2_feature)
        # attention_map,_ = self.co_attention(fusion_feat)
        # view1_feature = torch.add(attention_map,view1_feature)
        # view2_feature = torch.add(attention_map,view2_feature)
        # view2_feature = self.linearLayer2(view2_feature)

        view1_predict = self.classifier(view1_feature)
        view2_predict = self.classifier(view2_feature)
