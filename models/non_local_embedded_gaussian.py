...
class _NonLocalBlockND(nn.Module):
    ...
    def forward(self, x, return_nl_map=False):
        ...
        g_x = g_x.permute(0, 2, 1) # (b, c, d)->(b, d, c)
        theta_x = self.theta(x).view(batch_size, self.inter_channels, -1)
        theta_x = theta_x.permute(0, 2, 1)  # (b, hw, c)->(b, c, hw)->(b, hw, c) 
        phi_x = self.phi(x).view(batch_size, self.inter_channels, -1)   # (b, hw, c)->(b, c, hw)->(b, hw, c) 描述中

        f = torch.matmul(theta_x, phi_x)
        f_div_C = F.softmax(f, dim=-1)

        y = torch.matmul(f_div_C, g_x)
        y = y.permute(0, 2, 1).contiguous() # 强制拷贝出一份tensor
